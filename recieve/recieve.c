...
#include <sqlite3.h>
/*#include <stropts.h>        // Catching stdin buffer for exiting*/
...
int main(int argc , char *argv[])
{
    ...
    char buffer[DATA_SIZE];  //data buffer of DATA_SIZE bytes
    ...
    char station;   // Holds station name from sent data
    char* mac;      // Holds mac address from sent data
    char* t;        // Holds time from sent data
    char* power;    // Holds power from sent data
    char sql_in[71];
    int n;
    ...
    while(TRUE)
    {
        ...
        for (i = 0; i < max_clients; i++)
        {
            ...
            if (FD_ISSET( sd , &readfds))
            {
                ...
                //incoming message
                if ((valread = read( sd, buffer, DATA_SIZE)) == 0)
                {
                ...
                else
                {
                    ...
                    pch = strtok( buffer, "/" );
                    station = pch[0]; // Gets station letter from token
